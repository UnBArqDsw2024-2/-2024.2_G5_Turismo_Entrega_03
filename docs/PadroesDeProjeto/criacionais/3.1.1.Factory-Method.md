...
-->
Para a elaboração deste documento inicialmente, foi realizada uma reunião com os membros da equipe do projeto EvenTour para discutir e alinhar as principais questões relacionadas ao desenvolvimento do projeto. Na reunião, foi definido que as demandas seriam separadas para duplas, e cada um poderia ir fazendo um padrão de projeto criacional conforme a disponibilidade, devido ao tempo apertado.

Pesquisas foram conduzidas utilizando método Pomodoro para estudos. Usamos conteúdos disponibilizados na página da disciplina no Aprender3 e o site Refactoring.Guru, proporcionando uma base sólida para a análise e discussão.

Para produzir o conteúdo, realizamos videochamadas pelo Microsoft Teams (gravadas) para discutir ideias e possíveis utilizações do Factory Method no projeto. Juntos, produzimos três versões de modelagem em reuniões diversas. Após isso, desenvolvemos o código base e, posteriormente, um código mais concreto rodando no backend. O trabalho de documentação foi dividido para ser realizado de forma assíncrona.
